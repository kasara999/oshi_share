...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!recipientToken) {
      return NextResponse.json({ error: "受取人トークンがありません" }, { status: 400 });
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: "マッチング中にエラーが発生しました" }, { status: 500 });
    }
    ...
    return NextResponse.json({ match_id: matchId, card_id: cardId, card: cardData }, { status: 200 });
